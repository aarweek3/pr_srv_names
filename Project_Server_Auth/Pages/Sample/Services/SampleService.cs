...
namespace pr_srv_names.Pages.Sample.Services
{
    ...
    public class SampleService : ISampleService
    {
        ...
        private async Task ValidateNameUniqueness(string name, int? excludeId = null)
        {
            ...
            if (!isUnique)
            {
                _logger.LogWarning("Sampleа с именем '{Name}' уже существует. CorrelationId: {CorrelationId}", name,
                    correlationId);
                throw new ConflictException($"Sampleа с именем '{name}' уже существует.", "Sample", "Name");
            }
        }
        ...
        /// <returns>Sampleа</returns>
        private async Task<DAL.Models.Sample> GetSampleOrThrow(int id)
        {
            var correlationId = GetCorrelationId();
            var entity = await _sampleRepository.GetByIdAsync(id);
            if (entity == null)
            {
                _logger.LogWarning("Sample с ID {Id} не найдена. CorrelationId: {CorrelationId}", id, correlationId);
                throw new NotFoundException("Sample", $"Sample с ID {id} не найдена.");
            }
        ...
        /// </summary>
        public async Task<IEnumerable<SampleDetailDto>> GetAllCategoriesAsync()
        {
            var correlationId = GetCorrelationId();

            try
            {
                // Просто получаем все sample
        ...
        public async Task<SampleDetailDto?> GetSampleByIdAsync(int id)
        {
            ...
            try
            {
                ...
                if (entity == null)
                {
                    _logger.LogWarning("Sample с ID {Id} не найдена. CorrelationId: {CorrelationId}", id,
                        correlationId);
                    throw new NotFoundException("Sample", $"Sample с ID {id} не найдена.");
                }

                var result = _mapper.Map<SampleDetailDto>(entity);
                _logger.LogInformation("Sample с ID {Id} успешно получена. CorrelationId: {CorrelationId}", id,
                    correlationId);
                return result;
            }
        ...
        public async Task<SamplePagedResponseDto> GetAllCategoriesAsync(SamplePageRequestDto request)
        {
            var correlationId = GetCorrelationId();
        ...
        public async Task<SampleDetailDto> CreateSampleAsync(SampleCreateRequestDto request)
        {
            ...
            // Нормализация description перед валидацией
            ...
            try
            {
                ...
                var entity = _mapper.Map<DAL.Models.Sample>(request);
                await _sampleRepository.AddAsync(entity);
                await _unitOfWork.SaveChangesAsync();

                var result = _mapper.Map<SampleDetailDto>(entity);
                _logger.LogInformation("Sample с ID {Id} успешно создана. Name: {Name}. CorrelationId: {CorrelationId}",
        ...
        public async Task<SampleDetailDto> UpdateSampleAsync(int id, SampleUpdateRequestDto request)
        {
            ...
            try
            {
                // Используем общий метод для получения sample
                ...
                var result = _mapper.Map<SampleDetailDto>(entity);
                _logger.LogInformation(
                    "Sample обновлена - ID: {Id}, Name: {OldName} -> {NewName}. CorrelationId: {CorrelationId}",
        ...
        public async Task<bool> DeleteSampleAsync(int id)
        {
            ...
            try
            {
                ...
                _logger.LogInformation("Sample с ID {Id} успешно удалена. Name: {Name}. CorrelationId: {CorrelationId}",
                    entity.Id, entity.Name, correlationId);
                return true;
            }
            catch (Exception ex) when (!(ex is NotFoundException || ex is InvalidParametersException))
        ...
        public async Task<IEnumerable<SampleDetailDto>> GetCategoriesWithDescriptionAsync()
        {
            var correlationId = GetCorrelationId();
        ...
        public async Task<SampleDetailDto?> GetSampleByNameAsync(string name)
        {
            ...
            try
            {
                ...
                if (entity == null)
                {
                    _logger.LogWarning("Sample с именем '{Name}' не найдена. CorrelationId: {CorrelationId}", name,
                        correlationId);
                    throw new NotFoundException("Sample", $"Sample с именем '{name}' не найдена.");
                }

                var result = _mapper.Map<SampleDetailDto>(entity);
                _logger.LogInformation("Sample с именем '{Name}' успешно найдена. CorrelationId: {CorrelationId}", name,
                    correlationId);
                return result;
            }
            catch (Exception ex) when (!(ex is NotFoundException || ex is InvalidParametersException))
        ...
        public async Task<IEnumerable<SampleDetailDto>> SearchCategoriesByNameAsync(string searchTerm)
        {
            var correlationId = GetCorrelationId();
        ...
        /// <summary>
        /// Построение фильтра поиска по имени sample
        /// </summary>
        /// <param name="searchTerm">Поисковый термин</param>
        /// <returns>Expression для фильтрации или null</returns>
        private Expression<Func<DAL.Models.Sample, bool>>? BuildSearchFilter(string? searchTerm)
        {
            if (string.IsNullOrWhiteSpace(searchTerm))
                return null;
        ...
        private Expression<Func<DAL.Models.Sample, object>> BuildSortExpression(SampleSortField sortBy)
        {
